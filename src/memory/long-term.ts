...
const MEMORY_FACT_COLUMNS = 'id, tenant_id, chat_id, user_id, category, key, value, confidence, salience_score, source, recall_count, last_recalled_at, created_at, updated_at';
...
async function ensureTenantVectorIndex(tenantId: string): Promise<{ store: VectorStore; rebuilt: boolean } | null> {
  ...
  const factCount = Number((db.prepare(
    'SELECT COUNT(*) AS count FROM memory_facts WHERE tenant_id = ?',
...
function enqueueFactEmbedding(fact: MemoryFact): void {
  ...
  task = (async () => {
    ...
    .catch((err) => {
      ...
      }, 'Memory fact vector indexing failed');
    })
    .finally(() => {
...
export function saveFact(
  ...
  if (writePolicy === 'first_write_wins') {
    db.prepare(`
      INSERT INTO memory_facts (tenant_id, chat_id, user_id, category, key, value, confidence, salience_score, source)
      VALUES (?, ?, ?, ?, ?, ?, 1.0, ?, ?)
      ON CONFLICT(tenant_id, chat_id, category, key) DO NOTHING
    `).run(tenantId, chatId, userId ?? null, category, key, value, initialSalience, source ?? null);
  } else {
    db.prepare(`
      INSERT INTO memory_facts (tenant_id, chat_id, user_id, category, key, value, confidence, salience_score, source)
      VALUES (?, ?, ?, ?, ?, ?, 1.0, ?, ?)
...
export function getFacts(
  ...
  if (category) {
    return db.prepare(`
      SELECT ${MEMORY_FACT_COLUMNS}
      FROM memory_facts
      WHERE tenant_id = ? AND chat_id = ? AND category = ?
        ${access.sql}
...
export function getAccessibleFacts(
  ...
  return db.prepare(`
    SELECT ${MEMORY_FACT_COLUMNS}
    FROM memory_facts
    WHERE tenant_id = ?${catClause}${access.sql}
    ORDER BY category, salience_score DESC, updated_at DESC
...
function memorySearchTerms(query: string): string[] {
  return uniqueStrings(query.toLowerCase().match(/[a-z0-9]+|[\u4e00-\u9fff\u3400-\u4dbf]/g) ?? [])
    .slice(0, 24);
}
...
export function searchSemanticFacts(
  ...
  let hits = rows.map(row => {
    ...
    return { fact, score: 1 / (1 + rank), source: fact.source, timestamp: fact.updated_at };
  });

  // unicode61 does not segment every CJK phrase the same way users type it.
  // A scoped substring fallback keeps Chinese memory search useful without an
  // external tokenizer or embedding dependency.
  if (hits.length === 0 && terms.length > 0) {
    const likeTerms = terms.slice(0, 8);
    const clauses = likeTerms.map(() => `lower(f.key || ' ' || f.value) LIKE ?`).join(' OR ');
    const likeParams = likeTerms.map(term => `%${term.toLowerCase()}%`);
    const fallbackRows = db.prepare(`
      ...
      LIMIT ?
    `).all(tenantId, ...access.params, ...likeParams, effectiveTopK) as Array<Record<string, unknown>>;
    hits = fallbackRows.map(row => {
      const fact = rowToFact(row);
      const haystack = `${fact.key} ${fact.value}`.toLowerCase();
      const matched = likeTerms.filter(term => haystack.includes(term)).length;
      return { fact, score: matched / likeTerms.length, source: fact.source, timestamp: fact.updated_at };
    });
  }
...
export function consolidateEpisodes(
  ...
  const rows = db.prepare(`
    ...
    FROM memory_facts
    WHERE tenant_id = ? AND chat_id != '__semantic__'

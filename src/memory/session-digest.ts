 ...
 * summarizing what happened. Digests are searchable via semantic similarity.
 */
...
Include emotional context if relevant. Be concise.`,
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO session_digests (tenant_id, session_id, user_id, digest, topics, open_threads, message_count, session_start, session_end, vector_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
export function getRecentDigests(
  ...
  const rows = db.prepare(`
    SELECT id, tenant_id, session_id, user_id, digest, topics, open_threads,
           message_count, session_start, session_end, vector_json, created_at
...
export function searchDigests(
  ...
  const queryTerms = [...new Set(query.toLowerCase().match(/[a-z0-9]+|[\u4e00-\u9fff\u3400-\u4dbf]/g) ?? [])];

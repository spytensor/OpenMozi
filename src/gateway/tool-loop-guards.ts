...
export type LoopPatternType = 'consecutive' | 'periodic';
...
export class LoopDetector {
  ...
  constructor(opts?: { maxHistory?: number; consecutiveThreshold?: number }) {
  ...
  record(toolCalls: ToolCall[]): LoopPattern | null {
    ...
    const nonPolling = toolCalls.filter(tc => !POLLING_TOOLS.has(tc.function.name));
    if (nonPolling.length === 0) return null; // Pure polling — skip detection

...
const BrainStateConfigSchema = z.object({
  /** Enable brain state checkpoint extraction during compression. */
  enabled: z.boolean().default(true),
  /** Model for LLM reasoning extraction. 'auto' = use summary model from model-router. */
  extraction_model: z.string().default('auto'),
  /** Maximum tokens for the snapshot system message. */
  max_snapshot_tokens: z.number().int().min(100).max(2000).default(500),
  /** Persist snapshots to event_log for crash recovery. */
  persist_to_db: z.boolean().default(true),
  /** Which watermark levels trigger brain state extraction. */
  extract_on: z.array(z.enum(['soft', 'hard', 'rotate', 'in_loop'])).default(['soft', 'hard', 'rotate']),
});
...
const MoziConfigSchema = z.object({
  ...
  brain_state: BrainStateConfigSchema.default(() => ({
    enabled: true,
    extraction_model: 'auto',
    max_snapshot_tokens: 500,
    persist_to_db: true,
    extract_on: ['soft' as const, 'hard' as const, 'rotate' as const],
  })),
  model_router: ModelRouterConfigSchema.optional(),
  model_discovery: ModelDiscoveryConfigSchema.optional(),
  coding_worker: z.object({
    routing: z.enum(['auto', 'claude_code', 'codex_cli', 'gemini_cli']).default('auto'),
    available: z.array(z.enum(['claude_code', 'codex_cli', 'gemini_cli'])).default([]),
  }).optional(),

...
export function initTel(): void {
  ...
  registerExecutor('shell', 'execute', async (params, context) => {
    const { command, timeout, cwd, restricted, enforceWorkspaceBoundary } = params as {
      command: string;
      timeout?: number;
      cwd?: string;
      restricted?: boolean;
  ...
  registerExecutor('shell', 'execute_background', async (params, context) => {
    const { command, cwd, restricted, enforceWorkspaceBoundary, chat_id, tenant_id } = params as {
      command: string;
      cwd?: string;
      restricted?: boolean;
      enforceWorkspaceBoundary?: boolean;

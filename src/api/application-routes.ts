...
import { createReadStream, createWriteStream, existsSync, mkdirSync, readdirSync, realpathSync, renameSync, rmSync, statSync, unlinkSync, type Stats } from 'node:fs';
...
import { hasAnyPairedUsers, getAllowedUsers, validatePairingToken, addAllowedUser, createPairingToken } from '../security/pairing.js';
...
function resolveFallbackChatRoleSlot(
  ...
  for (const provider of getChatRoleEligibleProviders()) {
    if (!providerHasConfiguredKey(provider.id, rawProviders, storedKeyProviders)) continue;
    return {
...
function resolveChatRoleSlot(
  ...
  if (provider && isChatRoleEligibleProvider(provider)) {
    ...
    return {
      provider: provider.id,
      model,
      ready: providerHasConfiguredKey(provider.id, rawProviders, storedKeyProviders),
      eligible: true,
    };
  }
  ...
  return {
    ...resolveFallbackChatRoleSlot(rawProviders, storedKeyProviders),
    configured: {
      provider: providerId,
      model: modelId,
      eligible: false,
      reason: `${roleName} provider is not eligible for chat roles; cli-pipe providers are managed workers only.`,
    },
  };
}
...
function getModelRolesForTenant(tenantId: string): { brain: ModelRoleSlot; light: ModelRoleSlot; step: ModelRoleSlot; plan_summary: ModelRoleSlot; embedding: ModelRoleSlot } {
  ...
  return {
    brain: resolveChatRoleSlot('brain', brainProvider, brainModel, rawProviders, storedKeyProviders),
    light: resolveChatRoleSlot('light', lightProvider, lightModel, rawProviders, storedKeyProviders),
    step: legacyRoles?.step?.provider && legacyRoles.step.model
      ? resolveChatRoleSlot('step', legacyRoles.step.provider, legacyRoles.step.model, rawProviders, storedKeyProviders)
    ...
    plan_summary: legacyRoles?.plan_summary?.provider && legacyRoles.plan_summary.model
      ? resolveChatRoleSlot('plan_summary', legacyRoles.plan_summary.provider, legacyRoles.plan_summary.model, rawProviders, storedKeyProviders)
      : { provider: '', model: '', ready: true, eligible: true, inherit: true },
    embedding: {
...
export async function registerApiRoutes(
  ...
  // Plan DAGs owned by this session — read from the tasks table (the single
  // source of truth for plan state). The UI execution panel rehydrates from
  // this endpoint and live-updates from task_update websocket events.
  app.get('/api/sessions/:id/plans', async (request, reply) => {
    ...
    const { listPlanRootTasks, listTasks } = await import('../store/task-dag.js');
    const { loadTaskMetadata } = await import('../tasks/workspace.js');
    const plans = listPlanRootTasks(tenantId, { limit: 20 })
      .filter((root) => loadTaskMetadata(root.id)?.session_id === id)
      .map((root) => ({
        id: root.id,
        goal: root.objective || root.title,
        status: root.status,
        created_at: root.created_at,
        updated_at: root.updated_at,
        steps: listTasks({ tenant_id: tenantId, parent_task_id: root.id }).map((step) => ({
          id: step.id,
          title: step.title,
          status: step.status,
          guard_reason: typeof step.constraints.guard_reason === 'string' ? step.constraints.guard_reason : undefined,
          blocked_reason: typeof step.constraints.blocked_reason === 'string' ? step.constraints.blocked_reason : undefined,
        })),
      }));
    return reply.send({ sessionId: id, plans });
  });

  // Cancel a specific plan owned by this session: request cancellation of the
  // plan root's incomplete children. Cancellation flows through the same
  // registry used by /cancel (task-cancellation.ts): running tasks get their
  // abort signal, pending tasks get a pending request so they never start.
  // The panel refetches /plans afterwards to reflect the cancelled state.
  app.post('/api/sessions/:id/plans/:planId/cancel', async (request, reply) => {
    const tenantContext = getRequestTenantContext(request as { tenantContext?: { tenant_id: string; user_id: string; roles: string[] } });
    const tenantId = tenantContext?.tenant_id ?? 'default';
    const { id, planId } = request.params as Record<string, string>;
    if (!enforceSessionOwnership(reply, id, tenantId, tenantContext?.user_id)) return reply;

    const { getById, listTasks } = await import('../store/task-dag.js');
    const { loadTaskMetadata } = await import('../tasks/workspace.js');
    const { requestTaskCancellation } = await import('../core/task-cancellation.js');

    const root = getById(planId, tenantId);
    // The plan must exist AND belong to this session — never let a session
    // cancel another session's plan by guessing a planId.
    if (!root || loadTaskMetadata(planId)?.session_id !== id) {
      return reply.code(404).send({ error: 'Plan not found for this session' });
    }

    const TERMINAL = new Set(['completed', 'failed', 'cancelled']);
    const children = listTasks({ tenant_id: tenantId, parent_task_id: planId });
    const incomplete = children.filter((step) => !TERMINAL.has(step.status));

    const cancelled: string[] = [];
    for (const step of incomplete) {
      const result = await requestTaskCancellation(step.id, {
        tenantId,
        requestedBy: tenantContext?.user_id ?? 'ui',
        reason: 'user_cancel_plan',
        chatId: id,
      });
      if (result.ok) cancelled.push(step.id);
    }

    return reply.send({ sessionId: id, planId, cancelled, cancelledCount: cancelled.length });
  });

  app.post('/api/sessions/:id/plans/:planId/steps/:stepId/retry', async (request, reply) => {
    const tenantContext = getRequestTenantContext(request as { tenantContext?: { tenant_id: string; user_id: string; roles: string[] } });
    const tenantId = tenantContext?.tenant_id ?? 'default';
    const { id, planId, stepId } = request.params as Record<string, string>;
    if (!enforceSessionOwnership(reply, id, tenantId, tenantContext?.user_id)) return reply;

    const { getById, getDownstreamTasks, updateStatus, updateTask } = await import('../store/task-dag.js');
    const { query: queryEvents } = await import('../store/events.js');
    const { loadTaskMetadata } = await import('../tasks/workspace.js');
    const { isPlanRunActive, restartPlanFromMetadata } = await import('../core/plan-runner.js');
    ...
    const step = getById(stepId, tenantId);
    if (!step || step.parent_task_id !== planId) {
      return reply.code(404).send({ error: 'Step not found for this plan' });
    }
    if (step.status !== 'failed' && step.status !== 'cancelled') {
      return reply.code(409).send({ error: 'Plan step can only be retried when status is failed or cancelled' });
    }
    if (isPlanRunActive(planId)) {
      return reply.code(409).send({ error: 'Plan is already running; retry after the active run finishes' });
    }

    if (typeof step.constraints.guard_reason === 'string') {
      const nextConstraints = { ...step.constraints };
      delete nextConstraints.guard_reason;
      updateTask(stepId, { constraints: nextConstraints }, tenantId);
    }
    updateStatus(stepId, 'ready', tenantId, { reason: 'user_retry_plan_step' });
    updateStatus(planId, 'ready', tenantId, { reason: 'user_retry_plan_step', step_id: stepId });

    // Restore only descendants that never ran because this upstream step
    // failed. New rows carry explicit causality; the event-log fallback also
    // repairs plans created before blocked_by_task_id was introduced.
    const restoredDescendantIds: string[] = [];
    const queue = [stepId];
    const seen = new Set(queue);
    while (queue.length > 0) {
      const upstreamId = queue.shift()!;
      for (const dependent of getDownstreamTasks(upstreamId, tenantId)) {
        if (!seen.has(dependent.id)) {
          seen.add(dependent.id);
          queue.push(dependent.id);
        }
        const events = queryEvents('task', dependent.id, tenantId);
        const latestFailure = [...events].reverse().find((event) => event.event_type === 'task_failed');
        const payload = latestFailure?.payload && typeof latestFailure.payload === 'object'
          ? latestFailure.payload as Record<string, unknown>
          : {};
        const legacyDependencyFailure = typeof payload.error === 'string'
          && payload.error.startsWith('Dependency failed:');
        const causedByRetriedStep = dependent.constraints.blocked_by_task_id === stepId;
        if (dependent.status !== 'blocked'
          && !(dependent.status === 'failed' && (causedByRetriedStep || legacyDependencyFailure))) {
          continue;
        }
        const nextConstraints = { ...dependent.constraints };
        delete nextConstraints.blocked_by_task_id;
        delete nextConstraints.blocked_reason;
        updateTask(dependent.id, { constraints: nextConstraints }, tenantId);
        updateStatus(dependent.id, 'ready', tenantId, {
          reason: 'upstream_retry_restored',
          upstream_task_id: stepId,
        });
        restoredDescendantIds.push(dependent.id);
      }
    }

    // A retry run needs a brain fallback client like boot resume gets: role
    // routing covers step execution, but the completion summary silently
    // degrades to the compact template without a Brain client.
    let fallbackClient;
    try {
      const { getBrainClient } = await import('../core/model-router.js');
      fallbackClient = getBrainClient({ tenantId, userId: tenantContext?.user_id }).client;
    } catch {
      fallbackClient = undefined; // step role routing still executes the plan
    }
    const result = restartPlanFromMetadata(planId, {
      tenantId,
      systemPrompt: '',
      fallbackClient,
    });
    if (!result.started) {
      return reply.code(409).send({ error: `Plan retry could not start: ${result.reason ?? 'start refused'}` });
    }

    return reply.send({ ok: true, restoredDescendantIds });
  });
  ...
  app.get('/api/providers', async (request, reply) => {
    ...
    const providers = await Promise.all(getChatRoleEligibleProviders().map(async (provider) => {
      ...
      const hasKey = storedKeys.has(provider.id) || !!apiKey || !!resolveApiKey(provider.id, rawProviders);
      let discovery: ModelDiscoveryResult = hasKey || provider.id === configuredBrainProvider
        ...
        : {
            supported: provider.apiMode !== 'cli-pipe' && provider.apiMode !== 'bedrock-converse-stream',
            source: 'catalog',
            fetchedAt: null,

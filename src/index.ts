...
import { loadSystemPrompt, resolveWorkspaceDir, resolveTenantId, adaptPromptForChannel } from './system-prompt.js';
...
import { cancelAllRunningTurns, getRunningTurnCount } from './core/turn-cancellation.js';
import { createDefaultFailoverManager } from './core/provider-failover.js';
import { setFailoverManager } from './core/model-router.js';
...
export function createMessageHandler(
  ...
  handlerRef = async (msg: IncomingMessage): Promise<string | null> => {
    const executeMessage = async (): Promise<string | null> => {
      ...
      const systemPromptContent = adaptPromptForChannel(resolveSystemPrompt(tenantId), msg.channelType);
      ...
      if (msg.isCommand) {
        ...
        case 'help':
          return handleHelp();
        case 'steer': {
          // #257 — mid-run agent nudge. Queues for the next tool-call iteration
          // without opening a new user turn / breaking prompt cache.
          const { enqueueSteer } = await import('./gateway/steer-store.js');
          const steerText = msg.commandArgs ?? '';
          const result = enqueueSteer(msg.chatId, steerText, tenantId);
          if (result.accepted) {
            // §10 Fallback Discipline — if the brain has been idle past the
            // prompt-cache TTL, surface that as a visible hint instead of
            // silently paying the cache miss.
            if (result.brainIdle) {
              return '✓ Steer queued.\n⚠️ The brain has been idle past the prompt-cache TTL; this steer will start a fresh turn instead of reusing cached context.';
            }
            return '✓ Steer queued — the brain will see it at the next tool-call boundary.';
          }
          const reasonText = ({
    ...
    if (msg.isCommand && msg.command === 'cancel') {
      logger.info({ chatId: msg.chatId, userId: msg.userId }, 'Bypassing chat queue for /cancel command');
...
async function main(): Promise<void> {
  ...
  const startBackgroundExecutor = async (): Promise<void> => {
    ...
    bgRunner = new BackgroundJobRunner();
    bgRunner.start();
      ...
      void (async () => {
        try {
          ...
          const report = resumeIncompletePlans({
            ...
            systemPrompt: loadSystemPrompt(config, runtimeTenantId),
            fallbackClient: llmClient,
          });

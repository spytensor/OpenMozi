 ...
 * artifacts. `getSkillNodeModulesDir` / `getSkillPythonDir` are injected onto
 * NODE_PATH / PYTHONPATH for every shell command so scripts the Brain writes can
 * `require()` / `import` provisioned packages without a local install.
 */
...
/** pip `--target` root where skill python dependencies are installed. */
export function getSkillPythonDir(): string {

...
import { getModel as getRegisteredModel } from './providers.js';
...
/**
 * Normalize system messages for providers that don't support multiple system
 * messages interleaved with user/assistant messages (e.g. MiniMax).
 * Merges all system messages into a single one at the beginning.
 */
export function consolidateSystemMessages(messages: ModelMessage[], providerName?: string): ModelMessage[] {
  // OpenAI Responses/Chat APIs preserve interleaved system/developer messages.
  // Keeping per-turn context near the tail protects the append-only history
  // prefix. Providers such as MiniMax still require one leading system block.
  if (providerName === 'openai') return messages;
...
export function createAIAdapter(
  ...
  return {
    ...
    async chat(messages: ChatMessage[], chatOptions?: ChatOptions): Promise<ChatResponse> {
      ...
      const coreMessages = consolidateSystemMessages(toCoreMessages(messages), providerName);
      ...
      if (chatOptions?.tools && chatOptions.tools.length > 0) {
        const moziTools = toAITools(chatOptions.tools);

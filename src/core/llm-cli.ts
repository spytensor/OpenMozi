...
const INTERACTIVE_TIMEOUT_MS = 45_000;
...
function resolveTimeoutMs(
  ...
  if (executionScope === 'interactive') {
    return Math.min(resolved ?? INTERACTIVE_TIMEOUT_MS, INTERACTIVE_TIMEOUT_MS);
  }
...
async function execCli(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Strip session-detection env vars from all CLI providers to prevent
    // "nested session" blocks when spawning from within an active CLI session.
    // Claude Code: CLAUDECODE, CLAUDE_CODE_ENTRYPOINT
    // Codex CLI:   CODEX_CLI_SESSION, CODEX_SANDBOX_ACTIVE
    // Gemini CLI:  GEMINI_CLI_SESSION
  ...
  return {
    ...
    async chat(messages: ChatMessage[], options?: ChatOptions): Promise<ChatResponse> {
      ...
      const runOnce = async (attemptSessionId?: string): Promise<ChatResponse> => {
        ...
        const useStdin =
          backend.input === 'stdin' ||
          (backend.maxPromptArgChars && effectivePrompt.length > backend.maxPromptArgChars);

        if (!useStdin && backend.promptArg) {
          baseArgs.push(backend.promptArg, effectivePrompt);
        } else if (!useStdin) {
        ...
        const result = await execCli(
          backend.command,
          baseArgs,
          useStdin ? effectivePrompt : undefined,
          timeoutMs,

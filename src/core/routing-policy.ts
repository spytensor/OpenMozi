...
import { getProvider, resolveRuntimeModel } from './providers.js';
...
function isChatRoleEligibleEntry(entry: ModelCapabilityEntry): boolean {
  return entry.apiMode !== 'cli-pipe';
}

function isChatRoleEligibleProvider(providerId: string | undefined): boolean {
  if (!providerId) return false;
  return getProvider(providerId)?.apiMode !== 'cli-pipe';
}

...
import type { ModelDef, ProviderDef, ProviderApiMode, CliBackendConfig } from './providers.js';
...
export const PROVIDERS: Record<string, ProviderDef> = {
  ...
  // ── CLI-pipe providers ──────────────────────────────────────────────
  // CLI providers NEVER hardcode model IDs. The CLI tool owns its model
  // catalog; MOZI just passes through whatever the user sets in brain.model.
  // When brain.model is empty, --model is omitted so the CLI uses its own default.
  // A single generic template supplies metadata (context window, capabilities)
  // for internal bookkeeping — the template ID is never sent to the CLI.
  ...
  'claude-cli': defineProvider({
    ...
    models: [
      model({
        id: '_cli-default', name: 'Claude CLI (user-configured)', tier: 'high',
        contextWindow: 200_000, maxOutputTokens: 16_000,
  ...
  'codex-cli': defineProvider({
    ...
    models: [
      model({
        id: '_cli-default', name: 'Codex CLI (user-configured)', tier: 'high',
        contextWindow: 200_000, maxOutputTokens: 16_000,
        supportsTools: false, supportsStreaming: false, supportsVision: false,
      }),
    ],

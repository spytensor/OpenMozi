...
interface ArtifactContractResolution {
  directive?: string;
}

export function userExplicitlyRequestsArtifact(text: string): boolean {
  return explicitlyRequestsRenderableArtifact(text);
}

function responseDeclinesArtifact(text: string): boolean {
  return /无法|不能|没法|暂时.*不能|暂时.*无法|当前.*不支持|抱歉.*无法|sorry.*cannot|sorry.*unable|unable to|cannot|can't|failed to/i.test(text);
}

function buildArtifactRepairDirective(userText: string): string {
  const requiredType = explicitlyRequestedArtifactContentType(userText);
  const typeDirective = requiredType ? ` Set content_type to exactly "${requiredType}".` : '';
  return `[SYSTEM ARTIFACT CONTRACT] The user explicitly requested a renderable artifact and this channel supports artifacts. Your previous reply did not produce the requested artifact type. Continue the same request now. You MUST either call create_artifact with the full renderable content, or give a brief user-facing explanation that you cannot produce it right now.${typeDirective} Do not reply with a promise without calling the tool.`;
}

export function resolveArtifactContract(params: {
  userText: string;
  responseText: string;
...
}): ArtifactContractResolution | null {
  const {
    userText,
    responseText,
    artifactCapable,
  ...
  if (!artifactCapable || artifactEmitted || !userExplicitlyRequestsArtifact(userText)) {
    return null;
  }
  const trimmed = responseText.trim();
  if (trimmed && responseDeclinesArtifact(trimmed)) {
    return null;
  }
  if (artifactRepairAttempts < maxArtifactRepairAttempts) {
    return { directive: buildArtifactRepairDirective(userText) };
  }
  // The sanctioned single autonomous repair retry already ran and still failed.
  // Do NOT fabricate a canned "no artifact was produced, please retry" message —
  // the constitution forbids fallback narration that hides runtime truth. Return
  // the model's real response unmodified (the guard above guarantees `trimmed`
  // is non-empty, so we never surface an empty turn here).
  return null;
}

...
import { isBrainStateMessage } from './brain-state.js';
...
export async function compress(
  ...
  // Filter out brain state messages — they are protected and must not be compressed
  const toCompress: ChatMessage[] = [];
  const brainStateTurns: ChatMessage[] = [];
  for (const t of earlyTurns) {
    if (isBrainStateMessage(t)) {
      brainStateTurns.push(t);
    } else {
      toCompress.push(t);
    }
  }
  const keptTurns = [...brainStateTurns, ...turns.slice(splitIndex)];

...
function classifyProviderError(message: string, code?: string): { kind: ProviderErrorKind; retryable: boolean } {
  ...
  if (/rate.?limit|too many requests|throttl/.test(text)) {
    return { kind: 'rate_limit', retryable: true };
  }
  if (/timeout|timed?.?out|\b5\d{2}\b|server.?error|service.?unavailable|bad.?gateway|overloaded|econnreset|econnrefused/.test(text)) {

...
/**
 * Report that a subagent has been spawned.
 */
export function reportAgentSpawned(agentId: string, role: string, taskId?: string): void {
  emit({
    type: 'agent_spawned',
    agentId,
    agentRole: role,
  ...
  logger.info({ agentId, role, taskId }, 'Agent spawned');
}

/**
 * Report that a subagent has completed its work.
 */
export function reportAgentCompleted(agentId: string, summary?: string): void {
  emit({
  ...
  logger.info({ agentId, summary }, 'Agent completed');
}

/**
 * Report that a subagent has failed.
 */
export function reportAgentFailed(agentId: string, error: string): void {
  emit({
    type: 'agent_failed',
    agentId,
    error,
...
/**
 * Report a token budget warning.
 * @param level - 'soft' | 'hard' | 'rotate'
 * @param usagePercent - Current usage as a percentage (0-100)
 */
export function reportBudgetWarning(level: string, usagePercent: number): void {
  emit({
    type: 'budget_warning',
    level,
    usagePercent,
  ...
  logger.warn({ level, usagePercent }, 'Budget warning');
}
...
export function formatProgressText(event: ProgressEventInput): string {
  switch (event.type) {
    ...
    case 'project_mode_kickoff':
      return event.card ? `Project Mode started` : 'Project Mode started';

    case 'project_plan_updated':
      return `Project plan updated`;

    case 'team_assignment': {
      const who = event.agentRole ?? 'team member';
      const model = event.model ? ` -> ${event.model}` : '';
      const reason = event.routingReason ? ` (${event.routingReason})` : '';
      return `Team: ${who}${model}${reason}`;
    }

    case 'milestone_reached':
      return `Milestone reached: ${event.milestoneLabel ?? 'unknown'}`;

    case 'blocker_detected':
      return `Blocker: ${event.description ?? 'unknown'}`;

    case 'risk_detected':
      return `Risk: ${event.description ?? 'unknown'}`;

    case 'next_step_updated':
      return `Next: ${event.summary ?? 'updated'}`;

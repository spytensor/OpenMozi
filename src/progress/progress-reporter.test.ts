...
describe('progress/progress-reporter', () => {
  describe('DAG tracking lifecycle', () => {
    ...
    it('reportTaskCompleted updates state and emits task_completed + overall_progress', () => {
      ...
      reportTaskCompleted('dag-1', 't1', 'Analyze code', 1200);
  ...
  describe('agent events', () => {
    it('reportAgentSpawned emits agent_spawned', () => {
      const events: ProgressEvent[] = [];
      on((e) => events.push(e));

      reportAgentSpawned('agent-1', 'researcher', 't1');

      expect(events).toHaveLength(1);
      expect(events[0].type).toBe('agent_spawned');
      expect(events[0].agentId).toBe('agent-1');
      expect(events[0].agentRole).toBe('researcher');
      expect(events[0].taskId).toBe('t1');
    });

    it('reportAgentCompleted emits agent_completed', () => {
      const events: ProgressEvent[] = [];
      on((e) => events.push(e));

      reportAgentCompleted('agent-1', 'Found 3 results');

      expect(events).toHaveLength(1);
      expect(events[0].type).toBe('agent_completed');
      expect(events[0].agentId).toBe('agent-1');
      expect(events[0].summary).toBe('Found 3 results');
    });

    it('reportAgentFailed emits agent_failed', () => {
      const events: ProgressEvent[] = [];
      on((e) => events.push(e));

      reportAgentFailed('agent-1', 'OOM');

      expect(events).toHaveLength(1);
      expect(events[0].type).toBe('agent_failed');
      expect(events[0].agentId).toBe('agent-1');
      expect(events[0].error).toBe('OOM');
    });
  });

  describe('budget warning', () => {
    it('reportBudgetWarning emits budget_warning', () => {
      const events: ProgressEvent[] = [];
      on((e) => events.push(e));

      reportBudgetWarning('soft', 75);

      expect(events).toHaveLength(1);
      expect(events[0].type).toBe('budget_warning');
      expect(events[0].level).toBe('soft');
      expect(events[0].usagePercent).toBe(75);
    });
  });

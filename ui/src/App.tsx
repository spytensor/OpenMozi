...
const ExecutionPlanPanel = lazy(() => import("@/components/chat/ExecutionPlanPanel").then((module) => ({ default: module.ExecutionPlanPanel })));
 ...
 * still fits beside it. Drag the panel wide — or shrink the window — until the
 * chat would fall below MIN_CHAT_WIDTH, and it undocks to the full-width
 * takeover (chat hidden) instead of cramming both. This is a pure function of
 * the desired panel width and the available main area, so it reacts to BOTH the
 * resize drag and window/sidebar changes.
 */
function isArtifactDocked(panelWidth: number, sidebarCollapsed: boolean, viewportWidth?: number): boolean {
...
function clampArtifactPanelWidth(width: number, sidebarCollapsed = false, viewportWidth?: number): number {
  // Free drag: bounded only by the available main area (never wider than the
  // window, never below the panel's own minimum). Dragging past the point where
  // the chat gets too thin doesn't cap here — it flips the panel to the
  // full-width takeover via isArtifactDocked().
  const maxWidth = Math.max(ARTIFACT_PANEL_MIN_WIDTH, mainAreaWidth(sidebarCollapsed, viewportWidth));
...
function activeTurnIds(timeline: TimelineItem[]): Set<string> {
  const ids = new Set<string>();
  for (const item of timeline) {
    if (item.type !== "task_update") continue;
    const task = item.data as TaskUpdate;
    if ((task.status === "running" || task.status === "pending") && task.turnId) {
      ids.add(task.turnId);
    }
  }
  return ids;
...
function shouldAutoOpenArtifact(artifact: Artifact, activeTurns: Set<string>, dismissedArtifactId: string | null): boolean {
  if (artifact.id === dismissedArtifactId) return false;
  if (artifact.status === "running") return true;
  const turnId = artifactTurnId(artifact);
  return Boolean(turnId && activeTurns.has(turnId));
}
...
export default function App() {
  ...
  const handleWSMessage = useCallback((msg: WSInboundMessage) => {
    ...
    // Artifacts land as inline cards in the timeline. Current-turn artifacts
    // auto-open into the workspace so generation is visible, while a manual
    // close suppresses that same artifact from reopening.
  }, [chat.handleWSMessage, chat.addMessage, chat.setCurrentModel, chat.adoptResolvedSession, workspace.handleWSMessage, session.updateSessionTitle, session.updateSessionActivity, session.adoptSession, t]);
  ...
  useEffect(() => {
    if (activeArtifact || chat.sessionState === "IDLE") return;
    const latest = [...chat.timeline].reverse().find((item) => item.type === "artifact")?.data as Artifact | undefined;
    if (!latest) return;
    // Active turns: real running work in the timeline (background jobs) plus the
    // live turn tracked from runtime signals — lifecycle rows no longer exist.
    const activeTurns = activeTurnIds(chat.timeline);
    if (chat.activeTurnId) activeTurns.add(chat.activeTurnId);
    if (shouldAutoOpenArtifact(latest, activeTurns, dismissedArtifactId)) {
      setActiveArtifact(latest);
    }
  }, [activeArtifact, chat.sessionState, chat.timeline, chat.activeTurnId, dismissedArtifactId]);
  ...
  const handleCloseArtifact = useCallback(() => {
    setDismissedArtifactId(activeArtifact?.id ?? null);
    setActiveArtifact(null);
  }, [activeArtifact?.id]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-base">
        <div className="text-center">
          <p className="text-xl font-bold text-accent mb-2">{t("app.productName")}</p>
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              {contentView === "chat" && (
                showEmptyWorkspace ? (
                  ...
                  <>
                    {session.timelineHasMore && (
                      <button
                        type="button"
                        onClick={() => void handleLoadOlderTimeline()}
                        disabled={session.timelineLoadingOlder}
                        className="mx-auto mt-2 rounded-md px-3 py-1.5 text-xs text-ink/55 hover:bg-ink/[0.04] disabled:opacity-50"
                      >
                        {t(session.timelineLoadingOlder ? "chat.history.loadingOlder" : "chat.history.loadOlder")}
                      </button>
                    )}
                    <ChatView
                      sessionId={session.activeSessionId}
                    ...
                    <Suspense fallback={null}>
                      <ExecutionPlanPanel sessionId={session.activeSessionId} timeline={chat.timeline} />
                    </Suspense>
              ...
              {contentView === "scheduled" && (
                <Suspense fallback={<LazySurfaceFallback />}><ScheduledView /></Suspense>
              )}
            </div>
          </div>
        </main>

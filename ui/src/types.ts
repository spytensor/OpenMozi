...
export interface TimelineItem {
  eventId?: number;
  type: "message" | "tool_event" | "task_update" | "approval_request" | "artifact" | "memory_update";
  timestamp: number;
  ...
  seq?: number;
  data: ChatMessage | ToolEvent | TaskUpdate | ApprovalRequest | Artifact | MemoryUpdate;
}
...
export type WSInboundMessage =
  ...
  | { type: "tool_event"; phase: "start" | "end"; tool: string; status?: string; intent?: string; result?: string; error?: string; elapsed_ms?: number; callId: string; taskId?: string; turnId?: string; seq?: number; agentId?: string; skillName?: string; skillDescription?: string; skillLoadOutcome?: "success" | "not_found" | "ineligible"; skillMissingBins?: string[]; skillMissingEnv?: string[]; skillLoadError?: string; timestamp?: number; sessionId?: string }
  | { type: "task_update"; task_id: string; parentTaskId?: string; rawStatus?: string; title: string; status: string; progress?: number; turnId?: string; seq?: number; timestamp?: number }

...
function unwrapMemoryFacts(
  ...
  return (facts ?? []).flatMap((entry) => {
    if ("fact" in entry) return [entry.fact];
    return [entry];
  });
}
    ...
    <span className="inline-flex items-center gap-1" title={`${label} — ${hint}`}>
      <span className="flex items-end gap-0.5" aria-hidden="true">
        {[1, 2, 3].map((bar) => (
          <span
            key={bar}
            className={bar <= level ? "w-0.5 rounded-full bg-accent/60" : "w-0.5 rounded-full bg-ink/15"}
        ...
        <div className="flex flex-col gap-2 rounded-lg border p-3" style={{ borderColor: "var(--border-subtle)", background: "var(--surface-input)" }}>
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              className="inline-flex h-7 items-center gap-1.5 rounded-md bg-accent px-3 text-[12px] font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50"
            >
            ...
            <section key={group.category}>
              ...
              <div className="overflow-hidden rounded-lg border" style={{ borderColor: "var(--border-subtle)", background: "var(--surface-elevated)" }}>
                {group.items.map((fact) => (
                  <article key={fact.id} className="border-b px-3 py-2.5 last:border-b-0" style={{ borderColor: "var(--border-subtle)" }}>
                    {editingId === fact.id ? (

              ...
              {loading ? (
                ...
                entries.map((entry) => (
                  <tr key={entry.id} className="border-t border-ink/[0.045]">
                    ...
                    <Td>{entry.details && Object.keys(entry.details).length > 0 ? <details className="max-w-[300px]"><summary className="cursor-pointer text-[11.5px] text-accent/80">{t("admin.audit.viewDetails")}</summary><pre className="mt-1 max-h-40 overflow-auto whitespace-pre-wrap break-all rounded-md bg-ink/[0.035] p-2 text-[10.5px] text-ink/52">{JSON.stringify(entry.details, null, 2)}</pre></details> : <span className="text-ink/28">—</span>}</Td>
      ...
      <SettingsGroup icon={BarChart3} title={t("admin.usage.title")} description={t("admin.usage.description")}>
        <div className="space-y-3">
          ...
          <details className="rounded-lg border border-ink/[0.06] bg-ink/[0.018] px-3 py-2">
            <summary className="flex cursor-pointer list-none items-center gap-2 select-none text-[12px] font-medium text-ink/65"><Filter className="h-3.5 w-3.5 text-accent/75" />{t("admin.usage.filters")}</summary>
...
const DASHBOARD_COLORS = ["#3b82f6", "#22c55e", "#f59e0b", "#ec4899", "#06b6d4", "#a855f7"];
  ...
  return <div className="grid gap-3 xl:grid-cols-2">
    <DashboardChartCard title={t("admin.usage.tokenTrend")} icon={ChartNoAxesCombined} className="xl:col-span-2">
      {daily.length === 0 ? <DashboardEmptyState label={t("admin.usage.noUsage")} compact /> : <div className="h-56"><ResponsiveContainer width="100%" height="100%"><AreaChart data={daily} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        <defs><linearGradient id="usage-token-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.34} /><stop offset="95%" stopColor="#3b82f6" stopOpacity={0.02} /></linearGradient></defs>
        ...
        <Area type="monotone" dataKey="tokens" name={t("admin.usage.totalTokens")} stroke="#60a5fa" strokeWidth={2} fill="url(#usage-token-fill)" />
    ...
    <DashboardChartCard title={t("admin.usage.modelReliability")} icon={Bot}>
      {models.length === 0 ? <DashboardEmptyState label={t("admin.usage.noUsage")} compact /> : <div className="h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={models} layout="vertical" margin={{ top: 8, right: 12, left: 8, bottom: 0 }}>
        ...
        <Bar dataKey="calls" name={t("admin.usage.llmCalls")} fill="#60a5fa" radius={[0, 3, 3, 0]} />
...
function DashboardChartCard({ title, icon: Icon, children, className }: { title: string; icon: LucideIcon; children: ReactNode; className?: string }) {
  return <section className={cn("rounded-lg border border-ink/[0.06] bg-ink/[0.018] p-3", className)}><h3 className="mb-2 flex items-center gap-2 text-[12px] font-semibold text-ink/68"><Icon className="h-3.5 w-3.5 text-accent/80" />{title}</h3>{children}</section>;
}

function DashboardEmptyState({ label, compact = false }: { label: string; compact?: boolean }) {
  return <div className={cn("flex items-center justify-center rounded-md border border-dashed border-ink/[0.08] text-[12px] text-ink/38", compact ? "h-56" : "h-36")}>{label}</div>;
}
      ...
      <div className="grid gap-1 sm:grid-cols-2">
        {options.map((option) => (
          <label key={option.id} className="flex min-w-0 items-center gap-2 rounded-md px-2 py-1.5 text-[12px] text-ink/62 hover:bg-ink/[0.035]">
            <input type="checkbox" checked={selected.has(option.id)} onChange={() => toggle(option.id)} className="h-3.5 w-3.5 accent-[var(--accent)]" />
...
function SummaryTile({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
  return (
    <div className="rounded-lg border border-ink/[0.06] bg-ink/[0.018] p-3">
      <div className="flex items-center gap-1.5 text-[11px] text-ink/38"><Icon className="h-3.5 w-3.5 text-accent/75" />{label}</div>
  ...
  return (
    <div className="rounded-lg border border-ink/[0.06] bg-ink/[0.018] p-3">
      ...
      <div className="h-2 overflow-hidden rounded-full bg-ink/[0.06]">
        <div className="h-full rounded-full bg-accent/70" style={{ width: `${percent}%` }} />
      </div>
    </div>
  );
}
          ...
          {entries.map(([key, value]) => (
            <div key={key}>
              ...
              <div className="h-1.5 overflow-hidden rounded-full bg-ink/[0.06]">
                <div className="h-full rounded-full bg-accent/65" style={{ width: `${max > 0 ? Math.max(4, Math.round((value / max) * 100)) : 0}%` }} />
              </div>
            </div>
          ))}
        </div>

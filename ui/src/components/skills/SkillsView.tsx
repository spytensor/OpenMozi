...
function categoryAccentClass(category: CategorySectionKey): string {
  const classes: Record<CategorySectionKey, string> = {
    utility: "border-accent/15 bg-accent/10 text-accent",
    media: "border-warning/20 bg-warning/10 text-warning",
    coding: "border-success/20 bg-success/10 text-success",
    communication: "border-error/20 bg-error/10 text-error",
    research: "border-accent-light/15 bg-accent-light/10 text-accent-light",
    system: "border-ink/[0.12] bg-ink/[0.06] text-ink/62",
    other: "border-ink/10 bg-ink/[0.04] text-ink/52",
  };
...
function SkillIconTile({ skill, size = "md" }: { skill: SkillInfo | SkillDetail; size?: "md" | "lg" }) {
  ...
  return (
    <div
      data-testid={`skill-icon-${iconKey}`}
      className={cn(
        "flex flex-shrink-0 items-center justify-center rounded-md border",
        categoryAccentClass(category),
        size === "lg" ? "h-12 w-12 text-[24px]" : "h-11 w-11 text-[22px]",
      )}
      aria-hidden="true"
    >
      <Icon className={size === "lg" ? "h-5 w-5" : "h-4 w-4"} strokeWidth={2.1} />
    </div>
  );
}
...
function SkillCard({ skill, onOpen }: { skill: SkillInfo; onOpen: () => void }) {
  ...
  return (
    <button
      ...
      className="card-surface group min-w-0 p-4 text-left transition-all duration-180ms hover:-translate-y-0.5 hover:border-accent/35 hover:bg-elevated/80 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-accent/45"
    >
      <div className="flex min-w-0 items-start gap-3.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 flex items-center gap-2 text-[11.5px] text-ink/32">
            <span className="h-1.5 w-1.5 rounded-full bg-current opacity-55" />
            <span>{categorySectionLabel(category, t)}</span>
          </div>
        </div>
      </div>
    </button>
              ...
              {canEdit ? (
                <section className="space-y-2">
                  ...
                  <textarea
                    ...
                    spellCheck={false}
                    className="h-[320px] w-full resize-y rounded-md border border-ink/10 bg-[var(--code-bg)] px-3 py-2 font-mono text-[12px] leading-5 text-ink/72 outline-none focus:border-accent/50"
                  />
                </section>
              ) : null}
...
function MarkdownPreview({ content }: { content: string }) {
  return (
    <div className="rounded-md border border-ink/10 bg-ink/[0.025] px-4 py-3">
      <div className="prose prose-invert prose-sm max-w-none text-sm leading-relaxed [&_a]:text-accent [&_a]:underline [&_a]:underline-offset-2 [&_code]:font-mono [&_code]:text-xs [&_code]:text-accent-light [&_pre]:overflow-x-auto [&_pre]:rounded-md [&_pre]:border [&_pre]:border-ink/[0.06] [&_pre]:bg-[var(--code-bg)] [&_pre]:p-3 [&_pre]:text-xs [&_table]:my-3 [&_table]:block [&_table]:max-w-full [&_table]:overflow-x-auto [&_td]:border-b [&_td]:border-ink/[0.05] [&_td]:px-3 [&_td]:py-1.5 [&_th]:border-b [&_th]:border-ink/10 [&_th]:px-3 [&_th]:py-1.5 [&_th]:text-left">

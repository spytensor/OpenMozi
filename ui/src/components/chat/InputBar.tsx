                  ...
                  >
                    {cmds.map((c) => (
                      <CommandItem
                        ...
                        className="mx-1 flex cursor-pointer items-center gap-3 rounded-md px-3 py-2.5 data-[selected=true]:bg-accent/15 data-[selected=true]:text-ink"
                      >
                        <span className="shrink-0 font-mono text-sm font-medium text-accent">{c.cmd}</span>
        ...
        <div className={cn("relative", isEmptyVariant ? "px-4 pb-3 pt-3" : "px-3.5 pb-2.5 pt-2.5")}>
          {contextCompression && contextCompression.stage !== "completed" && (
            <div data-testid="context-compression-status" className={cn("mb-2 flex items-center gap-2 px-1 text-[11px]", contextCompression.stage === "failed" ? "text-red-500" : "text-ink/50")}>
              {contextCompression.stage !== "failed" && <span className="h-2 w-2 animate-pulse rounded-full bg-accent" />}
          ...
          <textarea
            ...
            className={cn(
              "block w-full resize-none bg-transparent pr-12 text-[15px] leading-relaxed focus:outline-none disabled:cursor-not-allowed",
          ...
          {(attachments.length > 0 || uploadError) && (
            <div className="mt-2 flex flex-wrap items-center gap-1.5 pr-12">
                ...
                >
                  <PopoverTrigger asChild>
                    <button
                      ...
                      style={{
                        background: addMenuOpen ? "var(--surface-active)" : "transparent",
                        color: addMenuOpen ? "var(--text-primary)" : "var(--text-muted)",
                      }}
                      onMouseEnter={(e) => {
                        if (disabled || uploading) return;
                      ...
                      onMouseLeave={(e) => {
                        if (disabled || uploading) return;
                        e.currentTarget.style.background = addMenuOpen ? "var(--surface-active)" : "transparent";
                        e.currentTarget.style.color = addMenuOpen ? "var(--text-primary)" : "var(--text-muted)";
                      }}
                    >
                      {uploading ? (
...
function AddMenu({
  ...
  const { t } = useLocale();
  const soonItems: Array<{ key: MessageKey; icon: typeof Terminal }> = [
    { key: "composer.add.terminal", icon: Terminal },
    { key: "composer.add.goal", icon: Target },
    { key: "composer.add.planMode", icon: ListChecks },
  ];
  ...
  return (
    <PopoverContent
      ...
      side="top"
      align="start"
      className="w-[264px] max-w-[calc(100vw-2rem)] p-1"
    >
      <button
        type="button"
        role="menuitem"
        onClick={onFiles}
        disabled={disabled}
        className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-50"
        style={{ color: "var(--text-primary)" }}
        onMouseEnter={(event) => {
          if (!disabled) event.currentTarget.style.background = "var(--surface-hover)";
        }}
        onMouseLeave={(event) => {
          event.currentTarget.style.background = "transparent";
        }}
      >
        <span className="flex h-4 w-4 shrink-0 items-center justify-center" style={{ color: "var(--text-secondary)" }}>
          {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Paperclip className="h-4 w-4" />}
        </span>
        <span className="min-w-0 flex-1 truncate text-[13px] font-medium">{t("composer.add.files")}</span>
      </button>
      <div className="mx-2.5 my-1 border-t" style={{ borderColor: "var(--border-subtle)" }} />
      <div className="px-2.5 pb-0.5 pt-1 text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: "var(--text-disabled)" }}>
        {t("composer.add.soon")}
      </div>
      {soonItems.map(({ key, icon: Icon }) => (
        <div
          key={key}
          aria-disabled="true"
          className="flex cursor-default items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-[12.5px]"
 ...
 * feedback: idle → accent arrow; working → stop square (reads as an action, not a
 * spinner); cancelling → disabled spinner until the runtime confirms the turn is
 * over. The stop state tints red on hover to telegraph "this interrupts".
 */
function SubmitButton({
  ...
  return (
    <button
      ...
      style={{
        background,
        color: working && !cancelling ? "var(--accent-fg)" : enabled ? "var(--accent-fg)" : "var(--text-muted)",
...
function PickerItem({
  ...
  return (
    ...
    >
      ...
      {active && <Check className="h-3.5 w-3.5" style={{ color: "var(--accent)" }} />}
    </button>
  );
}

  ...
  return (
    ...
    >
      {canResize && (
        ...
        >
          <span className="h-full w-px bg-ink/[0.08] transition-colors group-hover:bg-accent/60" />
      ...
      <div className="artifact-panel-header desktop-window-drag-region flex items-center gap-2 px-4 py-2.5 border-b border-ink/[0.06] bg-surface/95 backdrop-blur-sm shrink-0">
        ...
        {isRunning && (
          <div data-testid="artifact-live-status" className="ml-auto flex min-w-0 max-w-[42%] items-center gap-2 text-[11.5px] text-ink/42">
            ...
            <span className="shrink-0 rounded bg-accent/10 px-1.5 py-0.5 text-[10px] text-accent/70">
        ...
        <div className="flex items-center gap-1 ml-2">
          {kind === "code" && (
            <button
              ...
              className={cn("p-1.5 rounded-md transition-colors", showCode ? "text-accent bg-accent/10" : "text-ink/30 hover:text-ink/50")}
      ...
      <div data-testid="artifact-panel-content" className="min-h-0 flex-1 overflow-hidden">
        <ArtifactErrorBoundary resetKey={artifact.id}>
          {kind === "file" ? (
            <FileArtifactRenderer artifact={artifact} />
          ) : kind === "document" ? (
